...
export default async function handler(req, res) {
  ...
  try {
    ...
    let { data, error } = await supabase.from('cta_corriente_usuarios').select('id_usuario,nombre,es_yo').order('nombre', { ascending: true });
    if (error) {
      if (error.message && (error.message.includes('es_yo') || error.code === 'PGRST200' || error.message.includes('does not exist'))) {
        ...
        data = (fallbackRes.data || []).map(u => ({ ...u, es_yo: false }));
      } else {
        throw error;
      }
    }
