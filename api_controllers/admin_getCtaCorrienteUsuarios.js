...
export default async function handler(req, res) {
  ...
  try {
    ...
    let { data, error } = await supabase
      ...
      .select('id_usuario,nombre,es_yo,id_cuenta_principal')
      .eq('user_id', userId)
      .order('nombre', { ascending: true });
    if (error) {
      if (error.message && (error.message.includes('es_yo') || error.code === 'PGRST200' || error.message.includes('does not exist'))) {
        const fallbackRes = await supabase.from('cta_corriente_usuarios').select('id_usuario,nombre').eq('user_id', userId).order('nombre', { ascending: true });
