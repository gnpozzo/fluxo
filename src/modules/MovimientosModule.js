...
export class MovimientosModule extends BaseModule {
    ...
    this.#table = new App.DataTable(
      document.getElementById('mov-tabla-wrap'),
      {
        columns: [
          ...
          { key: '_acciones',       label: '',           align: 'right',
            exportable: false,
  ...
  _bindListeners() {
    document.getElementById('mov-btn-ingreso')
      ?.addEventListener('click', () => this.#abrirModalAlta('INGRESO'));
    document.getElementById('mov-btn-egreso')
      ?.addEventListener('click', () => this.#abrirModalAlta('EGRESO'));
  ...
  #renderAcciones(row) {
    const editDisabled = row.id_consumo_tarjeta_origen ? 'title="Editar desde Tarjetas"' : '';
    return `
      <div style="display:flex;gap:4px;justify-content:flex-end">
        <button class="btn-accion" data-action="edit" data-id="${row.id_movimiento}"
                ${editDisabled}>
          ${App.Icons.get('edit', 'icon-sm')}
        </button>
        <button class="btn-accion btn-danger" data-action="delete" data-id="${row.id_movimiento}">
  ...
  #handleAccion(action, row) {
    if (action === 'edit') {
      if (row.id_consumo_tarjeta_origen) {
        App.Toast.info('Editá este movimiento desde la sección Tarjetas.');
        return;
      }
